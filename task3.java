	...
	static Scanner in=new Scanner(System.in);
        ...
        static class Metro {
            public void solve() {
                ...
                pq.add(new pair2(st, d[st]));
                boolean visited[] = new boolean[n + 1];
                visited[st] = true;
                while (!pq.isEmpty()) {
                    pair2 p = pq.poll();
                    if (p.node == end) {
                        System.out.println(d[p.node]);
                        return;
                    }
                    for (pair pp : arrayList[p.node]) {
                        if (d[p.node] <= pp.t && d[pp.u] > d[p.node] + pp.w) {
                            d[pp.u] = d[p.node] + pp.w;
                            pq.add(new pair2(pp.u, d[pp.u]));
                        }
                    }
                }
            ...
            class pair {
                ...
                public pair(int u, long v, long w) {
                    this.u = u;
                    this.v = v;
                    this.w = w;
                }
